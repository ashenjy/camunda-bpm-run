...
/**
 * <p>
 * Authenticates a request against the JWT Token and JWT Validator.
 * A successful validation will generate a ValidatorResultJwt
 * object which is used to inject the username, group IDs, and tenant IDs into the engine's thread execution.
 * </p>
 *
 */
@CompileStatic
    ...
    public AuthenticationResultJwt extractAuthenticatedUser(HttpServletRequest request,
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith(JWT_AUTH_HEADER_PREFIX)) {
            ...
            try{
                ...
                validatorResult = validator.validateJwt(request, engine, encodedCredentials, jwtSecretPath)
            } catch(all){
